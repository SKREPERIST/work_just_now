...
// Page Login
    $(".page-login input:not([type=\"submit\"])").on('focus blur', function(){
...
// Floating Label Form
    ...
    $(function () {
        $('.input-s-t').floatLabels();
    });


    var $rows = $(".full-width table tbody tr td .chk-01:checked");
    $rows.each(function () {
        $(this).parent('td').parent('tr').addClass('row-checked');
    })

    $(".full-width table tbody tr td .chk-01").change(function() {
        if(this.checked) {
            $(this).parent('td').parent('tr').addClass('row-checked');
        } else {
            $(this).parent('td').parent('tr').removeClass('row-checked');
        }
    });


    $(".chk-02").change(function() {
        if(this.checked) {
            $(".chk-01").prop("checked", true);
            $(".full-width table tbody tr").removeClass('row-checked');
            $(".full-width table tbody tr").addClass('row-checked');
        } else {
            $(".chk-01").prop("checked", false);
            $(".full-width table tbody tr").removeClass('row-checked');
        }
    });
